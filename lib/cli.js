...
class CLI {
    ...
    async function viewByManager(db) {
        ...
        const managersSQL = 'SELECT id, first_name, last_name FROM employee WHERE manager_id is NULL;'
        db.query(managersSQL, (error, result) => {
                if (error) {
                    console.error('Unable to get managers: ');
                    reject(error);
                } else {
                    resolve(result);
                }
        ...
        const managersList = managers.map(employee => ({
            name: `${employee.first_name} ${employee.last_name}`,
            value: employee.id
        }))
        ...
        const employeeSQL = 'SELECT id, first_name, last_name FROM employee WHERE manager_id = ?';
        return new Promise((resolve, reject) => {
            db.query(employeeSQL, [choice.manager], (error, result) => {
                if (error) {
